...
#[cfg(test)] mod tests {
    ...
    fn keygen() {
        let kp = Keypair::new();
        println!("You've generated a new Solana wallet: {}", kp.pubkey().to_string()); println!(""); println!("To save your wallet, copy and paste the following into a JSON file:");
        println!("{:?}", kp.to_bytes()); 
    ...
    fn wallet_to_base58() {
        ...
        let stdin = io::stdin();
        let wallet = stdin.lock().lines().next().unwrap()
            .trim_start_matches('[')
            .trim_end_matches(']')
            .split(',')
    ...
    fn airdrop() {
        ...
        // Connection to Solana Devnet RPC Client
    ...
    fn transfer_sol() {
        ...
        let to_pubkey = Pubkey::from_str("8ReQ161Yuog9ZYyBmbi81ZpcVoVbeeyoKU3AiphC1t7R").unwrap();
